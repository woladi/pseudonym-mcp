...
- d2ee1b2: Recognize every locale pack by default, and make `mcp-config.json` reachable.
...
- cde948b: Documentation: adds three illustrations to the README — the mask/restore flow,
  the recognizer coverage across Poland, the EU and the US, and how a match's
  confidence is scored against the sensitivity thresholds.

## 0.8.1

### Patch Changes

- a1da7cd: Documentation: the contributor guide described the pre-0.8 rule API
  (`pattern`, `engines`, `toPatternDef`), the pipeline diagram still listed the
  original five entity types, and the limitations section said dates always reach
  the cloud. All three now match what the code does.
